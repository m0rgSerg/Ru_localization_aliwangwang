...
/***/ function(module, exports, __webpack_require__) {
	...
	var _modIndex = __webpack_require__(155);
	...
	window.onload = function () {
	    (0, _reactDom.render)(_react2['default'].createElement(_modIndex2['default'], null), document.getElementById('container'));
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _fileList = __webpack_require__(157);
	...
	/**
	 * bsend 发送还是接收 1表示发送 0 表示接收
	 * transdirection 1表示发送 2表示接收
	 * filetype 文件的三种类型 0表示在线 1表示离线 2表示文件夹
	 * status 文件的当前状态 0初始状态 1请求对方中 2对方拒绝了 3接收方接受 4文件传输中 5发送完毕 6对方取消掉了 7对方打开保存对话框 8对方关闭保存对话框 9未知错误
	 * 自己加了一种status 10 正在安全扫描 11 扫描成功且安全 12 扫描成功且不安全 13 断点续传
	 */
	...
	var Index = (function (_React$Component) {
	    ...
	    _createClass(Index, [{
	        ...
	        value: function events() {
	            ...
	            var binder = new _aliImSdk2['default'].Binder();

	            // 主面板打开 这个事件不能清除
	            _aliImSdk2['default'].on('IMNotifyChatDlgReActive', function () {
	                location.reload();
	            });

	            // 主面板关闭, 解除事件绑定
	            binder.on('IMNotifyMainPanelClose', function () {
	                binder.clear();
	            });

	            binder.on('im.uiutil.onConversationChange', function (data) {
	                var uid = undefined;
	                data = data.cid;
	                if (data.appkey && data.nick) {
	                    uid = data.appkey + data.nick;
	                } else if (data.tribeid) {
	                    uid = data.tribeid;
	                }
	            ...
	            binder.on('im.uiutil.onConversationClose', function (data) {
	                _this.setState({
	                    activeUser: ''
	                });
	            });
	            ...
	            // 左侧打开的联系人/群选中事件
	            binder.on('LEFT_SIDEBAR_USER_SELECTED', function (data) {
	                _this.setState({
	                    activeUser: data.uid
	                });
	            });
	            ...
	            // 左侧打开的联系人/群删除事件
	            _aliImSdk2['default'].on('LEFT_SIDEBAR_USER_REMOVED BIZ_NAV_SELECTED', function (data) {
	                _this.setState({
	                    activeUser: ''
	                });
	            });
	            ...
	            // 收到/发送文件通知
	            _aliImSdk2['default'].on('IMFileOnRecvNotify', function (data) {

	                var list = _this.state.list;

	                var f = list.filter(function (item) {
	                    return item.sessionid === data.sessionid;
	                });

	                if (f.length) return;

	                data.resumefile == 1 && data.filetype == 0 && (data.status = 13) && (data.process = data.progress);

	                list.push(data);

	                _this.setMyList(list);
	            });

	            _aliImSdk2['default'].on('IMFileOnSendNotify', function (data) {
	                var list = _this.state.list;
	            ...
	            var timer = null;
	            // 接收/发送文件进度
	            _aliImSdk2['default'].on('IMFileOnProcessNotify', function (data) {
	                var sessionid = data.sessionid,
	                    list = _this.state.list,
	                    file = list.filter(function (item) {
	                ...
	                if (file && file.length) {
	                    ...
	                    // 开始有进度了才表示开始
	                    !file.process && (updateNow = true);

	                    file.process = data.process;
	                    if (data.speed) {
	                        file.timeleft = file.filesize * (100 - data.process) / 100 / data.speed;
	                    ...
	                    updateNow && _this.setMyList(list);

	                    if (timer) return;

	                    timer = setTimeout(function () {
	                        timer = null;
	                        _this.setMyList(_this.list);
	                    }, 1000);
	                }
	            });

	            // 接收/Успешно отправлено通知
	            _aliImSdk2['default'].on('IMFileOnFinishNotify', function (data) {

	                var f = _this.state.list.filter(function (item) {
	                    return item.sessionid === data.sessionid;
	                });

	                if (f && f.length) {
	                    f[0].process = 100;
	                    _this.setMyList(_this.state.list);

	                    if (data.transdirection == 1) {
	                        _this.removeFile(data.sessionid);
	                    } else {
	                        f[0].status = 10;
	                    }
	                }
	            ...
	            // 安全校验完成
	            _aliImSdk2['default'].on('IMFileOnCheckResultNotify', function (data) {
	                _this.removeFile(data.sessionid);
	            });
	            ...
	            // 对方拒绝
	            // 自己取消
	            _aliImSdk2['default'].on('IMFileOnEventNotify', function (data) {
	                if (!data) return;
	            ...
	            // 文件状态
	            _aliImSdk2['default'].on('IMFileOnOnlineFileFolderEventNotify', function (data) {
	                var list = _this.list,
	                    f = list.filter(function (item) {
	                    return item.sessionid === data.sessionid;
	                });
	                if (!f || !f.length) return;

	                f[0].bsend = data.transdirection == 1 ? 1 : 0;
	                f[0].status = data.status;

	                _this.setMyList(list);
	            });

	            // 清除离线文件
	            _aliImSdk2['default'].on('IMFileOnClearOfflineFileSessionsNotify', function () {
	                var list = _this.list;
	                var newList = [];
	                ...
	                _this.setMyList(newList);
	            });

	            // 清除文件
	            _aliImSdk2['default'].on('IMFileOnClearFileSessionsNotify', function () {
	                var list = _this.list;
	                var newList = [];
	            ...
	            // 清除文件夹
	            _aliImSdk2['default'].on('IMFileOnClearFolderSessionsNotify', function () {
	                var list = _this.list;
	                var newList = [];
	            ...
	            // 清除所有文件
	            _aliImSdk2['default'].on('IMFileOnClearAllFileSessionsNotify', function () {
	                _this.setMyList([]);
	            });
	        }
	    }, {
	        key: 'removeFile',
	        value: function removeFile(sessionid) {
	        ...
	        key: 'openFileWindow',
	        value: function openFileWindow(open) {
	            _aliImSdk2['default'].invoke('chat.file.ShowFileTransWebview', { show: open });
	        }
	    }, {
	        key: 'setMyList',
	        value: function setMyList(r) {
	            r || (r = []);
	            this.list = r;
	            this.setState({
	                list: r
	        ...
	        key: 'getActiveChattingUser',
	        value: function getActiveChattingUser() {
	            return new Promise(function (resolve, reject) {
	                _aliImSdk2['default'].invoke('im.uiutil.GetCurrentConversationID', {}).then(function (data) {

	                    //myconsole.log('GetActiveID', data);
	                    // 传文件只有单聊才有,选中了其他的类型的而出现了这个页面,就是逻辑有问题;
	                    if (data && data.result && data.result.type === 1) {
	                        data = data.result;
	                        data.uid = data.appkey + data.nick;
	                        resolve(data);
	                    } else {
	                        //myconsole.error('传文件只有单聊才有,选中了其他的类型的而出现了这个页面,就是逻辑有问题;');
	                    }
	                });
	            });
	        }
	        ...
	        value: function getFileList() {
	            ...
	            this.getActiveChattingUser().then(function (data) {
	                _this2.setState({
	                    activeUser: data.uid
	                });

	                _aliImSdk2['default'].invoke('chat.file.GetAllFileSession', {}).then(function (data) {
	                    //myconsole.log('GetAllFileSession', data);

	                    if (!data || !data.length) return;

	                    data.forEach(function (item) {
	                        item.resumefile == 1 && item.filetype === 0 && (item.status = 13) && (item.process = item.progress);
	                    });

	                    _this2.setMyList(data);
	                });
	            });
	        }
	    }, {
	        ...
	        value: function render() {
	            ...
	            var list = this.state.activeUser && this.state.activeUser.split(':')[0] ? this.state.list.filter(function (item) {
	                return item.contact && item.contact.split(':')[0] === _this3.state.activeUser.split(':')[0];
	            }) : [];

	            this.openFileWindow(list.length ? 1 : 0);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _fileItem = __webpack_require__(158);
	...
	var FileList = (function (_React$Component) {
	    ...
	    _createClass(FileList, [{
	        key: 'removeFile',
	        value: function removeFile(sessionid) {
	            this.props.removeFile(sessionid);
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            ...
	            list.forEach(function (item) {
	                fileList.push(_react2['default'].createElement(_fileItem2['default'], _extends({
	                    key: item.sessionid,
	                    removeFile: _this.removeFile.bind(_this)
	                }, item)));
	            });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _operationConfig = __webpack_require__(159);

	var _operationConfig2 = _interopRequireDefault(_operationConfig);

	var _tipConfig = __webpack_require__(160);

	var _tipConfig2 = _interopRequireDefault(_tipConfig);

	var _$ROOTPMsgModUtil = __webpack_require__(161);

	var _$ROOTPMsgModUtil2 = _interopRequireDefault(_$ROOTPMsgModUtil);
	...
	var FileItem = (function (_React$Component) {
	    ...
	    function FileItem() {
	        ...
	        _get(Object.getPrototypeOf(FileItem.prototype), 'constructor', this).apply(this, arguments);
	    }

	    _createClass(FileItem, [{
	        key: 'clickHandler',
	        value: function clickHandler(o) {
	            var _this = this;
	            ...
	            if (o.type === 1) {
	                this.props.removeFile(this.props.sessionid);
	            } else if (o.type === 2) {
	                param = this.props;
	            }

	            //myconsole.log(o.cmd, param);
	            _aliImSdk2['default'].invoke('chat.file.' + o.cmd, param).then(function (data) {
	                //myconsole.log(o.cmd, data, o);
	                if (o.type == 2) {
	                    _this.props.removeFile(_this.props.sessionid);
	                }
	            });
	        }
	    }, {
	        ...
	        value: function render() {
	            ...
	            var status = this.props.status || 0,
	                ...
	                ext = fileName && fileName.split('.') || '',
	                size = _$ROOTPMsgModUtil2['default'].getSize(this.props.filesize),
	                process = this.props.process || 0,
	                w = { width: process + '%' },
	                tip = _tipConfig2['default'][status] && _tipConfig2['default'][status][this.props.filetype][this.props.bsend],
	                operation = _operationConfig2['default'][status] && _operationConfig2['default'][status][this.props.bsend] && _operationConfig2['default'][status][this.props.bsend][this.props.filetype],
	                oTpl = [],
	                bTpl = [];

	            ext = ext && this.props.filetype != 2 ? ext[ext.length - 1].toUpperCase() : 'папка';

	            bTpl.push(_react2['default'].createElement(
	                'div',
	                { key: 'progress-bar-wrap', className: 'progress-bar-wrap' },
	                _react2['default'].createElement('span', { className: 'progress-bar', style: w })
	            ));

	            if (process > 0 && process < 100 && this.props.timeleft) {
	                bTpl.push(_react2['default'].createElement(
	                    'div',
	                    { key: 'result-time-tip', className: 'result-tip' },
	                    '剩余:' + _$ROOTPMsgModUtil2['default'].getFormatTime(this.props.timeleft)
	                ));
	                bTpl.push(_react2['default'].createElement(
	                    'span',
	                    { key: 'result-speed-tip', className: 'result-tip' },
	                    _$ROOTPMsgModUtil2['default'].getSize(this.props.speed) + '/s'
	                ));
	                tip = '';
	            }

	            this.props.filetype == 1 && this.props.bsend == 0 && this.props.expiretime && bTpl.push(_react2['default'].createElement(
	                'div',
	                { className: 'expire-time', key: 'expire-time' },
	                _$ROOTPMsgModUtil2['default'].timeFormatter(this.props.expiretime, true) + 'зрелость'
	            ));
	            tip && bTpl.push(_react2['default'].createElement(
	                'div',
	                { key: 'result-tip', className: 'result-tip' },
	                tip
	            ));

	            operation && operation.length && operation.forEach(function (item) {
	                oTpl.push(_react2['default'].createElement(
	                    'a',
	                    { key: item.text, onClick: _this2.clickHandler.bind(_this2, item) },
	                    item.text
	                ));
	            });
	            ...
	            bTpl.push(_react2['default'].createElement(
	                'div',
	                { key: 'operation', className: 'operation' },
	                oTpl
	            ));
...
/***/ function(module, exports) {
	...
	exports['default'] = {
	    ...
	    0: {
	        ...
	        0: {
	            // 在线
	            0: [{ text: 'получить', cmd: 'AcceptFile' }, { text: 'Сохранить как', cmd: 'SaveAsFile' }, { text: 'Мусор', cmd: 'RejectFile', type: 1 }],
	            // 离线
	            1: [{ text: 'получить', cmd: 'AcceptFile' }, { text: 'Сохранить как', cmd: 'SaveAsFile' }, { text: 'Мусор', cmd: 'RejectFile', type: 1 }, { text: 'В следующий раз получить', cmd: 'DelayRecvFile', type: 1 }],
	            // 文件夹
	            2: [{ text: 'получить', cmd: 'AcceptFile' }, { text: 'Сохранить как', cmd: 'SaveAsFile' }, { text: 'Мусор', cmd: 'RejectFile', type: 1 }]
	        },
	        // 发送方
	        1: {
	            0: [{ text: 'Отправить офлайн-файлы', cmd: 'OnlineFileToOffline', type: 2 }, { text: 'отменить', cmd: 'CancelSendFile', type: 2 }],
	            1: [{ text: 'отменить', cmd: 'CancelSendFile', type: 2 }],
	            2: [{ text: 'отменить', cmd: 'CancelSendFile', type: 2 }]
	        }
	    },
	    ...
	    3: {
	        0: {
	            // 在线
	            0: [{ text: 'отменить', cmd: 'CancelRecvFile', type: 2 }],
	            // 离线
	            1: [{ text: 'отменить', cmd: 'CancelRecvFile', type: 2 }],
	            // 文件夹
	            2: [{ text: 'отменить', cmd: 'CancelRecvFile', type: 2 }]
	        },
	        1: {
	            // 在线
	            0: [{ text: 'отменить', cmd: 'CancelSendFile', type: 2 }],
	            // 离线
	            1: [{ text: 'отменить', cmd: 'CancelSendFile', type: 2 }],
	            // 文件夹
	            2: [{ text: 'отменить', cmd: 'CancelSendFile', type: 2 }]
	    ...
	    11: {
	        0: {
	            // 在线
	            0: [{ text: 'включать', cmd: 'OpenFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }],
	            // 离线
	            1: [{ text: 'включать', cmd: 'OpenFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }],
	            // 文件夹
	            2: [{ text: 'Открыть папку', cmd: 'OpenDir' }]
	        },
	        1: {
	            // 在线
	            0: [{ text: 'включать', cmd: 'OpenFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }],
	    ...
	    12: {
	        0: {
	            // 在线
	            0: [{ text: 'Удалить (рекомендуется)', cmd: 'DeleteFile' }, { text: 'включать', cmd: 'OpenFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }],
	            // 离线
	            1: [{ text: 'Удалить (рекомендуется)', cmd: 'DeleteFile' }, { text: 'включать', cmd: 'OpenFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }],
	            // 文件夹
	            2: [{ text: 'Удалить (рекомендуется)', cmd: 'DeleteFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }]
	        },
	        1: {
	            // 在线
	            0: [{ text: 'Удалить (рекомендуется)', cmd: 'DeleteFile' }, { text: 'включать', cmd: 'OpenFile' }, { text: 'Открыть папку', cmd: 'OpenDir' }],
	    ...
	    13: {
	        ...
	        0: {
	            // 在线
	            0: [{ text: 'Резюме', cmd: 'AcceptResumeFile' }, { text: 'Сохранить как', cmd: 'SaveAsFile' }, { text: 'Мусор', cmd: 'RejectFile', type: 1 }],
	            // 离线
	            1: [{ text: 'получить', cmd: 'AcceptFile' }, { text: 'Сохранить как', cmd: 'SaveAsFile' }, { text: 'Мусор', cmd: 'RejectFile', type: 1 }, { text: 'В следующий раз получить', cmd: 'DelayRecvFile', type: 1 }],
	            // 文件夹
	            2: [{ text: 'получить', cmd: 'AcceptFile' }, { text: 'Сохранить как', cmd: 'SaveAsFile' }, { text: 'Мусор', cmd: 'RejectFile', type: 1 }]
...
/***/ function(module, exports) {
	...
	exports['default'] = {
	    0: {
	        0: ['', 'Ожидание получения другой стороной...'],
	        1: ['', 'Ожидание получения сервером'],
	        2: ['', 'Ожидание получения другой стороной...']
	    },
	    1: {
	        0: ['', 'Ожидание получения другой стороной...'],
	        1: ['', 'Ожидание получения сервером'],
	    ...
	    2: {
	        0: ['Вы отказались', 'Другая сторона отказалась'],
	        1: ['', 'Другая сторона отказалась'],
	        2: ['Вы отказались', 'Другая сторона отказалась']
	    },
	    3: {
	        0: ['Получение...', 'Отправка...'],
	        1: ['', 'Расчет оставшегося времени...'],
	    ...
	    5: {
	        0: ['Успешно получено', 'Успешно отправлено'],
	        1: ['', 'Файл загружен на сервер и будет храниться у ваших друзей 7 дней.'],
	        2: ['Успешно получено', 'Успешно отправлено']
	    },
	    6: {
	        0: ['Вы отменили', 'Другой участник отменил'],
	        1: ['', ''],
	        2: ['Вы отменили', 'Другой участник отменил']
	    },
	    7: {
	        0: ['Экономия...', 'Другая сторона экономит...'],
	        1: ['', 'Сохранение на сервер...'],
	        2: ['Экономия...', 'Другая сторона экономит...']
	    },
	    8: {
	        0: ['', ''],
	        1: ['', ''],
	    ...
	    13: {
	        0: ['Осталось: неизвестно', 'Ожидание получения другой стороной...'],
	        1: ['', ''],
	        2: ['', '']
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = {
	    ...
	    timeFormatter: function timeFormatter(time, onlyDate) {
	        var date = new Date(parseInt(time) * 1000),
	            numberBeauty = function numberBeauty(num) {
	            num = parseInt(num);return num > 9 ? num : '0' + num;
	        },
	            y = date.getFullYear(),
	            m = numberBeauty(date.getMonth() + 1),
	            d = numberBeauty(date.getDate());
	        if (!onlyDate) {
	            var h = numberBeauty(date.getHours()),
	                M = numberBeauty(date.getMinutes()),
	                s = numberBeauty(date.getSeconds());
	        }

	        return onlyDate ? [y, m, d].join('/') : [y, m, d].join('-') + ' ' + [h, M, s].join(':');
	    },

	    append: function append(wraper, html) {

	        if (!wraper || !html) return;
	        ...
	        var children = [].concat(_toConsumableArray(div.childNodes));

	        children.forEach(function (node) {

	            //if(node && node.getElementsByTagName){
	            //    var font = [...node.getElementsByTagName('font')];
	            //
	            //    // 去掉空白或者内含空格的元素
	            //    font.forEach(f => {
	            //        var ih = f.innerHTML;
	    ...
	    preppend: function preppend(wraper, html) {
	        ...
	        wraper.insertBefore(fragment, wraper.firstChild);

	        return children;
	    },

	    setCache: function setCache(id, obj) {
	        var o = this.getCache(id) || {};

	        __CACHE__[id] = Object.assign(o, obj);
	    },

	    getCache: function getCache(id) {
	        return __CACHE__[id] || {};
	    },

	    clearCache: function clearCache(uid) {

	        //myconsole.log(uid);
	        if (uid) {
	            delete __CACHE__[uid];
	        } else {
	            __CACHE__ = {};
	        }

	        //myconsole.log(__CACHE__);
	    },
	    ...
	    getById: function getById(id, tag) {
	        var t = document.getElementsByTagName(tag || '*');

	        return [].concat(_toConsumableArray(t)).filter(function (n) {
	            return n.id === id;
	            // 一般对一个消息都只处理一次, 下次就不用再处理了
	            //if(n.id === id) {
	            //    if(n.getAttribute('_f_') == '1') {
	            //        return false;
	            //    }else {
	            //        n.setAttribute('_f_', 1);
	            //        return true;
	            //    }
	            //}
	            //return false;
	    ...
	    getByAttr: function getByAttr(attr, tag) {
	        var t = document.getElementsByTagName(tag || '*');
	    ...
	    getOffsetLeft: function getOffsetLeft(element, wraper) {
	        var actualLeft = element.offsetLeft,
	            current = element.offsetParent,
	            elementScrollLeft = (wraper || document.documentElement).scrollLeft;
	    ...
	    getOffsetTop: function getOffsetTop(element, wraper) {
	        var actualTop = element.offsetTop,
	            current = element.offsetParent,
	            elementScrollTop = (wraper || document.documentElement).scrollTop;
	    ...
	    parseXML: (function () {
	        var reg = /<([a-z_\-]+)>([^<]*)<\/[a-z_\-]+>/gi;

	        return function (str) {
	            var tmpJson = {},
	                tmpResult;
	            while ((tmpResult = reg.exec(str)) !== null) {
	                tmpJson[tmpResult[1]] = tmpResult[2];
	            }
	            reg.lastIndex = 0;
	            return tmpJson;
	        };
	    ...
	    setFileCache: function setFileCache(id, cache) {
	        __FILE_CACHE__[id] = cache;
	    },

	    getFileCache: function getFileCache(id) {
	        return __FILE_CACHE__[id];
	    },

	    clearFileCache: function clearFileCache(id) {
	        if (id) {
	            delete __FILE_CACHE__[id];
	        } else {
	            __FILE_CACHE__ = {};
	        }
	    },
	    ...
	    getFormatTime: function getFormatTime(time) {
	        time = parseInt(time);
	        var secSuffix = 'второй';
	        var minSuffix = 'Минуты';
	        var hourSuffix = 'час';
	        var daySuffix = 'день';

	        if (time < 60) {
	            return time + secSuffix;
	        }
	        ...
	        if (t < 60) {
	            return Math.floor(t) + minSuffix + time % 60 + secSuffix;
	        }
	        ...
	        if (t < 24) {
	            var h = Math.floor(t),
	                m = Math.floor(d / 60),
	                s = d % 60;
	            return h + hourSuffix + m + minSuffix + s % 60 + secSuffix;
	        }
	        ...
	        var d = Math.floor(t),
	            ...
	            s = Math.floor(d % 3600);
	        return d + daySuffix + h + hourSuffix + m + minSuffix + s % 60 + secSuffix;
	    },

	    setReadWithCache: function setReadWithCache(tid, type) {
	        var cache = this.getCache('unreadAtMsgCache'),
	            el,
	            rect,
	            isDiff = false,
	            infos = [];
	        for (var id in cache) {
	            el = document.getElementById(cache[id].id);
	            rect = el && el.getBoundingClientRect();
	            if (rect && rect.top >= 0 && rect.bottom <= window.innerHeight) {
	                //console.log(cache[id]);
	                infos.push({
	                    guuid: cache[id].id,
	                    senderid: cache[id].senderId,
	                    atinfo: parseInt(cache[id].atflag)
	                });
	                delete cache[id];
	                el.classList.remove('J_unreadAtMsg');
	                isDiff = true;
	            }
	        }
	        infos.length && this.setRead(tid, infos, type);
	        isDiff && this.setCache('unreadAtMsgCache', cache);
	    },

	    setReadWithNode: function setReadWithNode(tid, container, type) {
	        var nodes = container && container.querySelectorAll('.J_unreadAtMsg');
	        ...
	        [].concat(_toConsumableArray(nodes)).forEach(function (el) {
	            rect = el && el.getBoundingClientRect();
	            if (rect && rect.top > 0 && rect.bottom <= window.innerHeight) {
	                infos.push({
	                    guuid: el.id,
	                    senderid: el.getAttribute('data-uid'),
	                    atinfo: parseInt(el.getAttribute('data-atflag'))
	                });
	                el.classList.remove('J_unreadAtMsg');
	            } else {
	                !cache && (cache = {});
	                cache[el.id] = {
	                    id: el.id,
	                    senderId: el.getAttribute('data-uid'),
	                    atflag: parseInt(el.getAttribute('data-atflag'))
	                };
	            }
	        });

	        infos.length && this.setRead(tid, infos, type);
	        cache && this.setCache('unreadAtMsgCache', cache);
	    },

	    setRead: function setRead(tid, infos, type) {
	        _aliImSdk2['default'].invoke(API_CMD_MID[type] + 'SetTribeAtRead', {
	            tribeid: tid,
	            infos: infos
	        });
	    },
