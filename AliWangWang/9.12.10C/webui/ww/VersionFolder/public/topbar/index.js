...
/***/ function(module, exports, __webpack_require__) {
	...
	var _modIndex = __webpack_require__(239);
	...
	window.__WPO && __WPO.setConfig({
	    ...
	    sample: 100,
	    spmId: 'pcww9.0-topbar', // spmId必须设置,如果没写默认值，会自动抓页面的spm
	    retCode: {}
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getLoginInfo = function getLoginInfo() {
	    ...
	    return new Promise(function (resolve, reject) {
	        if (uid) {
	            ...
	            _aliImSdk2['default'].invoke('chat.contact.GetCUInfo', {}).then(function (data) {
	                uid = data.uid;
	                resolve(uid);
	            })['catch'](function () {
	                reject();
	...
	var GoldLog = function GoldLog(logkey, param) {
	    getLoginInfo().then(function (uid) {
	        var paramStr = '',
	            hold = +new Date(),
	            img = new Image();
	        ...
	        img.src = '//gm.mmstat.com/' + logkey + '?cache=' + Math.floor(Math.random() * (1111111 - 9999999) + 9999999) + '&wwnick=' + uid + paramStr;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var d        = __webpack_require__(188)
	  , callable = __webpack_require__(201)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var assign        = __webpack_require__(189)
	  , normalizeOpts = __webpack_require__(196)
	  , isCallable    = __webpack_require__(197)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _menuList = __webpack_require__(242);

	var _menuList2 = _interopRequireDefault(_menuList);

	var _eventEmitter = __webpack_require__(241);

	var _eventEmitter2 = _interopRequireDefault(_eventEmitter);

	var _ad = __webpack_require__(245);

	var _ad2 = _interopRequireDefault(_ad);

	var _$ROOTCUtilLog = __webpack_require__(173);

	var _$ROOTCUtilLog2 = _interopRequireDefault(_$ROOTCUtilLog);

	var GetBtnStatusCmd = {
	    IM: 'im.uiutil.AddContactStatus',
	    TM: 'im.uiutil.GetTribeMgrStatus',
	    Multi: ''
	};
	...
	var Index = (function (_React$Component) {
	    ...
	    _createClass(Index, [{
	        key: 'binder',
	        value: function binder(eventName, callback) {
	            if (this.ignoreAll) return;

	            return _aliImSdk2['default'].on(eventName, callback);
	        }
	    }, {
	        key: 'events',
	        value: function events() {
	            ...
	            /********************** 人员选择变更相关 **************************/
	            // 左侧打开的联系人/群选中事件
	            this.binder('im.uiutil.onConversationChange', function () {
	                _this.getActiveChattingUser();
	                _eventEmitter2['default'].emit('NAME_EDITING', false);
	            });

	            // 左侧打开的联系人/群删除事件
	            this.binder('LEFT_SIDEBAR_USER_REMOVED', function (data) {
	                if (data && data.uid === _this.state.user.uid) {
	                    _this.clearUser();
	                }
	            });
	            ...
	            /**
	             * 主面板关闭
	             * @callback data {closeall: int}
	             */
	            this.binder('im.uiutil.onConversationClose', function (data) {
	                if (data && data.closeall) {
	                    _this.clearUser();
	                    _this.ignoreAll = true;
	                }
	            });

	            // 主面板打开 主面板关闭时不能移除这个事件
	            _aliImSdk2['default'].on('IMNotifyChatDlgReActive', function () {
	                _this.ignoreAll = false;
	            });

	            /********************** 群信息变更相关 **************************/
	            /**
	             * 群信息变更事件
	             * @param {object} data {tribeid, <array>detail}
	             * @struct {object} detail[i] {key: number, value: *}
	             * 0: 未知, 1: 群名称, 2: 群签名, 3: 群描述, 4: 群关键字, 5: 群类型, 6: 群验证方式
	             * 7: 是否允许游客, 8: 禁止关键字搜索, 9: 设置加入群密码, 10: 群成员上限, 11: at all权限
	             */
	            this.binder('im.tribemgr.onTribeBaseInfoChange', function (data) {
	                var user = _this.state.user;

	                if (user.stype === 'IM' || data.tribeid.tribeid !== user.uid) return;

	                data.detail.forEach(function (item) {
	                    if (item.key === 1) {
	                        user.nick = item.value;
	                    } else if (item.key === 2) {
	                        user.signature = item.value;
	                    }
	                });
	            ...
	            // 群管理按钮
	            this.binder('TMNotifyRefreshTribeMgrButton', function (data) {
	                if (_this.state.user.stype === 'IM') return;
	                _this.setState({ btnStatus: !!data });
	            });

	            /********************** 单聊变更相关 **************************/
	            // 添加好友按钮显示隐藏事件通知
	            this.binder('IMOnAddContactBtnChange', function (data) {
	                if (_this.state.user.stype !== 'IM') return;
	                data = parseInt(data);
	                _this.setState({ btnStatus: !!data });
	            });
	            ...
	            /**
	             * 显示名变更事件
	             * @callback data {userid, display}
	             */
	            this.binder('im.contact.onContactDisplayChange', function (data) {
	                var user = _this.state.user;

	                if (user.stype !== 'IM') return;

	                data.some(function (item) {
	                    if (user.uid === item.userid.appkey + item.userid.nick) {
	                        user.nick = item.display;
	                        _this.setState({ user: user });
	                        return true;
	                    }
	                });
	            ...
	            /**
	             * В сети状态变更事件
	             * @callback data {userid, status}
	             */
	            this.binder('im.contact.onContactStatusChange', function (data) {
	                var user = _this.state.user;
	            ...
	            /**
	             * 联系人签名变更事件
	             * @callback data [{userid, signature}]
	             */
	            this.binder('im.contact.onContactSignatureChange', function (data) {
	                var user = _this.state.user;
	        ...
	        /**
	         * @callback data {uid, signature, stype, nick, site, status}
	         */
	    }, {
	        ...
	        value: function getActiveChattingUser() {
	            ...
	            // 获取当前联系人id及类型
	            // @callback baseData {appkey, info, nick, tribeid, type}
	            //      - type: 1 联系人, 2 群, 3 讨论组
	            _aliImSdk2['default'].invoke('im.uiutil.GetCurrentConversationID').then(function (baseData) {
	                var stype = ['IM', 'TM', 'Multi'][baseData.type - 1];
	                var originId = undefined;

	                if (stype === 'IM') {
	                    originId = { appkey: baseData.appkey, nick: baseData.nick };

	                    // 获取单个联系人基本信息
	                    _aliImSdk2['default'].invoke('im.conversation.GetContactInfo', originId).then(function (data) {
	                        _this2.setState({
	                            user: {
	                                uid: baseData.appkey + baseData.nick,
	                                nick: data.display || baseData.nick,
	                                signature: data.signature,
	                                site: baseData.appkey,
	                                status: data.status,
	                                originId: originId,
	                                stype: stype
	                            }
	                ...
	                } else if (stype === 'TM' || stype === 'Multi') {
	                    originId = { tribeid: baseData.tribeid };

	                    // 获取群&讨论组基本信息
	                    _aliImSdk2['default'].invoke('im.tribemgr.GetShallowTribe', originId).then(function (data) {
	                        _this2.setState({
	                            user: {
	                                uid: baseData.tribeid,
	                                nick: data.tribeid.display,
	                                signature: data.signature,
	                                originId: originId,
	                                stype: stype
	                            }
	                ...
	                _this2.getMenuBtnStatus(stype, originId).then(function (btnStatus) {
	                    _this2.setState({ btnStatus: btnStatus });
	                })['catch'](function (error) {
	                    // do nothing...
	                });
	            });
	        ...
	        key: 'getMenuBtnStatus',
	        value: function getMenuBtnStatus(type, originId) {
	            var cmd = GetBtnStatusCmd[type || ''];

	            return new Promise(function (resolve, reject) {
	                if (!cmd) return reject();

	                // @callback result boolean
	                _aliImSdk2['default'].invoke(cmd, originId).then(function (result) {
	                    resolve(result);
	                })['catch'](function (error) {
	                    reject();
	                });
	        ...
	        value: function changeName(name) {
	            (0, _$ROOTCUtilLog2['default'])('wwxb.21.20');

	            _aliImSdk2['default'].invoke('im.tribemgr.ChangeMultiName', {
	                tribeid: this.state.user.originId,
	                name: name
	            });
	        }
	    }, {
	        ...
	        value: function getUserRole() {
	            ...
	            _aliImSdk2['default'].invoke('im.uiutil.GetAliClerkFlag').then(function (flag) {
	                flag && _this3.setState({ isAli: true });
	            });
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            return _react2['default'].createElement(
	                ...
	                _react2['default'].createElement(
	                    ...
	                    _react2['default'].createElement(_user2['default'], _extends({}, this.state.user, { changeName: this.changeName.bind(this) })),
	                    _react2['default'].createElement(_menuList2['default'], this.state)
	                ),
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SITE_MAP = {
	    'cntaobao': 'Член Taobao',
	    'cnalichn': 'Член китайской станции',
	    'cnaliyun': 'Участник облака Alibaba',
	    'enaliint': 'Член международной станции'
	};
	...
	var User = (function (_React$Component) {
	    ...
	    _createClass(User, [{
	        ...
	        value: function componentDidMount() {
	            ...
	            _eventEmitter2['default'].on('NAME_EDITING', function (data) {
	                if (_this.state.editing === data) return;
	        ...
	        value: function shouldComponentUpdate(next, s) {
	            var _props = this.props;
	            var stype = _props.stype;
	            var site = _props.site;
	            var signature = _props.signature;
	            var nick = _props.nick;
	            var status = _props.status;

	            var editing = this.state.editing;
	            var shouldUpdate = editing !== s.editing || next.stype !== stype || next.status !== status || next.site !== site || next.signature !== signature || next.nick !== nick;

	            //myconsole.log('%c顶部用户名子模块是否需要更新:', 'background:yellow;color:#333;font-size:14px;', shouldUpdate);
	            return shouldUpdate;
	        }
	        ...
	        value: function clickHandler() {
	            ...
	            switch (this.props.stype) {
	                case 'IM':
	                    _aliImSdk2['default'].invoke('im.uiutil.ShowContactInfo', { userid: this.props.originId });
	                    break;
	                case 'TM':
	                    _aliImSdk2['default'].invoke('im.uiutil.TribeInfo', { tribeid: this.props.uid });
	                    break;
	                case 'Multi':
	                    // 如果是讨论组，则显示名变成编辑状态
	                    this.setState({ editing: true });
	                    setTimeout(function () {
	                        var input = _this2.refs.nameEditing;
	                        var v = input.value;
	                        // 使光标移动到最后
	                        input.value = '';
	                        input.focus();
	        ...
	        value: function blurHandler() {
	            ...
	            var value = this.refs.nameEditing.value.trim();
	        ...
	        value: function render() {
	            ...
	            var stype = _props2.stype;
	            var site = _props2.site;
	            var signature = _props2.signature;
	            var nick = _props2.nick;
	            var status = _props2.status;

	            var statusInfo = stype === 'IM' && _$ROOTCOnlinestatus2['default'][status],
	                icon = null,
	                cls = (0, _classnames2['default'])('name', { link: stype !== 'Multi' });

	            if (statusInfo && statusInfo.icon) {
	                icon = _react2['default'].createElement('img', { className: 'status', src: statusInfo.icon, title: statusInfo.text });
	            }

	            var s = site ? '[' + (SITE_MAP[site] || '会员') + ']' : '';
	            ...
	            return _react2['default'].createElement(
	                ...
	                _react2['default'].createElement(
	                    ...
	                    !this.state.editing ? _react2['default'].createElement(
	                        ...
	                        nick
	                    ) : _react2['default'].createElement('input', { className: 'name-editing',
	                        type: 'text',
	                        ref: 'nameEditing',
	                        defaultValue: nick,
	                ...
	                signature ? _react2['default'].createElement(
	                    'div',
	                    { className: 'signature' },
	                    signature
	                ) : null
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _eventEmitter = __webpack_require__(187);
	...
	var MenuList = (function (_React$Component) {
	    ...
	    _createClass(MenuList, [{
	        ...
	        value: function shouldComponentUpdate(next) {
	            ...
	            var shouldUpdate = user.stype !== next.user.stype || btnStatus != next.btnStatus;

	            //myconsole.log('%c 是否需要更新菜单列表', 'background:yellow;font-size:14px;', shouldUpdate);
	        ...
	        key: 'onMenuClick',
	        value: function onMenuClick(cmd, type, param) {
	            var _this = this;

	            if (type === 'Multi' && cmd === 'im.uiutil.MultiChatInvite') {
	                param = {
	                    tribeid: this.props.user.originId
	                };
	            }

	            if (type === 'IM' && cmd === 'im.uiutil.MultiChatInvite') {
	                param = {
	                    member: this.props.user.originId
	                };
	            }

	            if (type === 'TM') {
	                param = this.props.user.originId;
	            }

	            if (cmd == 'im.uiutil.ShowReportCenter') {
	                param = {
	                    params: 0,
	                    userid: this.props.user.originId
	                };
	            }

	            _aliImSdk2['default'].invoke(cmd, param).then(function () {
	                if (cmd === 'chat.conversation.CloseSingleConversation') {
	                    // 通知其他窗口
	                    _aliImSdk2['default'].invoke('util.FireJSEvent', {
	                        event: 'CLOSE_MULTI_CONVERSATION',
	        ...
	        value: function render() {
	            ...
	            var stype = this.props.user.stype;

	            var btnStatus = this.props.btnStatus;
	            var isAli = this.props.isAli;

	            var menuList;

	            if (!stype) return null;

	            if (stype === 'IM' || stype === 'TM') {
	                menuList = [].concat(btnStatus ? _config2['default'][stype]['visible'] : _config2['default'][stype]['hidden']);
	            } else {
	                menuList = [].concat(_config2['default'][stype]);
	            }
	            ...
	            return _react2['default'].createElement(
	                ...
	                menuList.map(function (menu) {
	                    return menu.isAli && isAli || !menu.isAli ? _react2['default'].createElement(_menuItem2['default'], _extends({ key: menu.cmd,
	                        onMenuClick: _this2.onMenuClick.bind(_this2),
	                        type: stype }, menu)) : null;
	                })
	            );
	        }
	    }]);
	...
	exports['default'] = {
	    IM: {
	        visible: [{
	            ...
	            cmd: 'im.uiutil.AddContact',
	            icon: 'icon-add-friends',
	            logkey: 'wwxb.2.7'
	        }, {
	            text: 'Отчет',
	            cmd: 'im.uiutil.ShowReportCenter',
	            icon: '',
	            showText: true
	            //logkey: 'wwxb.2.7'
	        }, {
	            isAli: true,
	            text: 'Пригласите присоединиться к группе обсуждения',
	            cmd: 'im.uiutil.MultiChatInvite',
	            icon: 'icon-icon',
	            logkey: 'wwxb.21.1'
	        }, {
	            text: 'Передача файлов',
	            cmd: 'im.uiutil.SendFiles',
	            icon: 'icon-file',
	            needXY: true,
	            logkey: 'wwxb.2.8'
	        }, {
	            text: 'Более',
	            cmd: 'im.uiutil.ToolBarMenu',
	            icon: 'icon-more',
	            needXY: true
	        }],
	        hidden: [{
	    ...
	    TM: {
	        hidden: [],
	        visible: [{
	            text: 'Групповые визитки',
	            cmd: 'im.uiutil.TribeInfo',
	            icon: 'icon-xitongmingpian'
	        }, {
	            text: 'Настройки группы',
	            cmd: 'im.uiutil.TribeMgr',
	            icon: 'icon-seting',
	            logkey: 'wwxb.7.7'
	        }]
	    },
	    ...
	    //{
	    //    text: '退出',
	    //    cmd: 'chat.conversation.CloseSingleConversation', //chat.multichat.ExitMultiChat
	    //    icon: 'icon-exit'
	    //}
	    ]
	};
	...
	var MenuItem = (function (_React$Component) {
	    ...
	    _createClass(MenuItem, [{
	        ...
	        value: function shouldComponentUpdate(next) {
	            ...
	            var shouldUpdate = icon !== next.icon || showText != next.showText || text !== next.text;

	            //myconsole.log('%c 是否需要更新菜单项', 'background:yellow;font-size:14px;', shouldUpdate);

	            return shouldUpdate;
	        }
	    }, {
	        key: 'menuClickHandler',
	        value: function menuClickHandler(ev) {
	            var param = {};

	            if (this.props.needXY) {
	                param.hit_x = ev.screenX;
	                param.hit_y = ev.screenY;
	            }

	            if (this.props.logkey) {
	                (0, _$ROOTCUtilLog2['default'])(this.props.logkey);
	            }

	            this.props.onMenuClick(this.props.cmd, this.props.type, param);
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            //myconsole.log('%c菜单 render', 'background:yellow;font-size:14px;');
	            return _react2['default'].createElement(
	                'span',
	                { className: 'menu-item', onClick: this.menuClickHandler.bind(this), title: this.props.text },
	                this.props.icon ? _react2['default'].createElement('i', { className: 'iconfont ' + this.props.icon }) : null,
	                this.props.showText ? _react2['default'].createElement(
	                    'span',
	                    { className: 'menu-text' },
	                    this.props.text
	                ) : ''
...
/***/ function(module, exports, __webpack_require__) {
	...
	var AD = (function (_React$Component) {
	    ...
	    function AD(props) {
	        ...
	        _get(Object.getPrototypeOf(AD.prototype), 'constructor', this).call(this, props);
	    ...
	    _createClass(AD, [{
	        ...
	        value: function events() {
	            ...
	            // 左侧联系人/群选中, 切换广告
	            _aliImSdk2['default'].on('im.bizutil.onConversationChange', function (data) {
	        ...
	        value: function getADList() {
	            ...
	            _aliImSdk2['default'].invoke('chat.ad.GetBannerInfo').then(function (data) {

	                if (data && data.adList && data.adList.length) {
	                    _this2.list = data.adList;
	                    _this2.setState({
	                        current: data.adList[0]
	                    });
	                    _this2.count = _this2.list.length * 10;
	                } else {
	                    _this2.setState({
	                        current: _this2.state.list && _this2.state.list[0]
	                    });
	                }
	            });
	        }
	        ...
	        value: function getUserRole() {
	            return new Promise(function (resolve, reject) {
	                _aliImSdk2['default'].invoke('im.uiutil.GetAliClerkFlag').then(function (data) {
	        ...
	        value: function componentDidMount() {
	            ...
	            this.getUserRole().then(function () {
	                _this4.getADList();
	                _this4.events();
	            });
	        }
	        ...
	        value: function render() {
	            ...
	            var ad = this.state.current;

	            return _react2['default'].createElement(
	                'div',
	                { className: 'ad-wrap' },
	                ad ? _react2['default'].createElement(
	                    'a',
	                    { href: ad.linkUrl, onClick: this.clickHandler.bind(this), target: '_blank' },
	                    _react2['default'].createElement('img', { src: ad.picUrl })
	                ) : null
	            );
	        }
	    }]);
