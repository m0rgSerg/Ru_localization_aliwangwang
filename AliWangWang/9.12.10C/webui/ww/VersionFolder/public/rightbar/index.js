...
/******/ 	__webpack_require__.p = "//g.assets.daily.taobao.net/aliww/client.pcww.mainpanel/0.0.1/";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _aliImSdk = __webpack_require__(156);

	var _aliImSdk2 = _interopRequireDefault(_aliImSdk);

	var _modIndex = __webpack_require__(223);

	var _modIndex2 = _interopRequireDefault(_modIndex);

	__webpack_require__(154);

	window.__WPO && __WPO.setConfig({
	    imgUrl: 'https://retcode.taobao.com/r.png?',
	    sample: 100,
	    spmId: 'pcww9.0-rightbar', // spmId必须设置,如果没写默认值，会自动抓页面的spm
	    retCode: {}
	});
	...
	if (window.DEBUG) {
	    window.SDK = _aliImSdk2['default'];
	}

	// 重写invoke方法
	var oldInvoke = _aliImSdk2['default'].invoke;
	_aliImSdk2['default'].invoke = function (api, param) {
	...
	document.addEventListener('DOMContentLoaded', function () {
	    _reactDom2['default'].render(_react2['default'].createElement(_modIndex2['default'], null), document.getElementById('container'));
	});
...
/***/ function(module, exports) {
	...
	window.CONSOLE_CACHE = [];

	window.myconsole = {
	    log: function log() {
	        if (!window.DEBUG) return;
	        var args = Array.prototype.slice.call(arguments);
	        CONSOLE_CACHE.push(args);
...
/***/ function(module, exports, __webpack_require__) {
	...
	!(function(global, factory) {
	    ...
	    global.onInvokeNotify = function(sid, status, response) {
	        ...
	        if (task.destroyed) return _errorLog('%cWARNGIN: sid: [' + sid + '] Соответствующая задача уничтожена!', 'color:#ff4400;font-weight:bold', task.config.fn);
...
/***/ function(module, exports, __webpack_require__) {
	...
	/**
	 * level: 0: 未知, 1: 群主, 2: 管理员, 3: 成员, 5: 游客
	 * >> 原数据：0: 群主, 1: 管理员, 2: 成员, 3: 游客
	 */
	...
	var _listTitle = __webpack_require__(224);

	var _listTitle2 = _interopRequireDefault(_listTitle);

	var _memberList = __webpack_require__(225);

	var _memberList2 = _interopRequireDefault(_memberList);

	var _multiMemberList = __webpack_require__(227);

	var _multiMemberList2 = _interopRequireDefault(_multiMemberList);

	var _search = __webpack_require__(229);

	var _search2 = _interopRequireDefault(_search);

	var _bulletin = __webpack_require__(234);
	...
	var Index = (function (_React$Component) {
	    ...
	    function Index(props) {
	        ...
	        this.getMoreMember = this.getMoreMember.bind(this);
	    ...
	    _createClass(Index, [{
	        ...
	        key: 'initTribeData',
	        value: function initTribeData() {
	            var _this = this;

	            _aliImSdk2['default'].invoke('chat.contact.GetCUID').then(function (uid) {
	                _this.loginId = uid;
	            })
	            // 获取当前选中目标 {<int>type, <string>appkey, <string>nick, <string>info, <string>tribeid}
	            .then(function () {
	                return _aliImSdk2['default'].invoke('im.uiutil.GetCurrentConversationID');
	            })
	            // 获取群信息
	            .then(function (data) {
	                if (data.type !== 2 && data.type !== 3) return;
	                _this.tribeid = data.tribeid;
	                _this.setState({ tribeid: data.tribeid });
	                return _this.updateTribeInfo(data.tribeid);
	            });
	        }
	    }, {
	        key: 'updateTribeInfo',
	        value: function updateTribeInfo(tribeid) {
	            var _this2 = this;

	            return _aliImSdk2['default'].invoke('im.tribemgr.GetShallowTribe', { tribeid: tribeid }).then(function (tribeInfo) {
	                var stype = tribeInfo.type === 0 ? 'TM' : 'Multi';
	                _this2.setState({
	                    originInfo: tribeInfo,
	                    role: tribeInfo.mylevel,
	                    stype: stype
	                });

	                _this2.getMemberList(tribeid, stype, _this2.loginId);
	                _this2.isTribe(stype) && _this2.getBulletinSrc(_this2.tribeid);
	            });
	        }
	    }, {
	        key: 'resetState',
	        value: function resetState(tribeid, stype) {
	            clearTimeout(this.updateTimmer);
	            clearTimeout(this.summaryTimmer);
	            this.updateTimmer = null;
	            this.summaryTimmer = null;
	        ...
	        value: function bindEvents() {
	            ...
	            this.binder = new _aliImSdk2['default'].Binder();

	            /**
	             * 断线重连事件，重新加载
	             */
	            this.binder.on('im.tribemgr.onTribeShallowLogin', function (data) {
	                location.reload();
	            });

	            /**
	             * 当前会话变更事件
	             * @callback data cid: {display, portrait, tribeid, type}
	             *           type: 0 biz, 1 contact, 2 tribe, 3 multi
	             */
	            this.binder.on('im.uiutil.onConversationChange', function (data) {
	                if (!data || !data.cid) return;
	                data = data.cid;
	                if (data.type !== 2 && data.type !== 3) return;
	                var stype = data.type === 2 ? 'TM' : 'Multi';

	                _this3.resetState(data.tribeid, stype);
	                _this3.updateTribeInfo(data.tribeid);
	            });

	            /**
	             * 群公告变更
	             * @callback data {<string>tribeid}
	             */
	            this.binder.on('im.tribemgr.onTribeBulletinUpdate', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid !== tribeid) return;

	                var frame = document.getElementById('J_bulletinFrame');
	                frame.src = frame.src;
	            });

	            /**
	             * 群成员状态变化
	             * @callback data {tribeid, userid, status}
	             */
	            this.binder.on('im.tribemgr.onTribeMemberStatusChange', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                data.memberid = data.userid.appkey + data.userid.nick;

	                var isUpdated = _this3.updateList(data, ['status']);
	                !isUpdated && _this3.updateCacheList(data, ['status']);
	            });

	            /**
	             * 群成员数量变化
	             * @callback data {tribeid, online, total}
	             */
	            this.binder.on('im.tribemgr.onTribeMemberCntChange', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;

	                _this3.online = data.online || _this3.online;
	                _this3.total = data.total || _this3.total;

	                if (_this3.summaryTimmer) return;
	                _this3.summaryTimmer = setTimeout(function () {
	                    _this3.summaryTimmer = null;
	                    _this3.setState({
	                        online: _this3.online,
	                        total: _this3.total
	                    });
	                }, 5000);
	            });
	            ...
	            /**
	             * 群成员信息(display)变化
	             * @callback data {tribeid, userid, display}
	             */
	            this.binder.on('im.tribemgr.onTribeMemberBaseInfoChange', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                data.memberid = data.userid.appkey + data.userid.nick;

	                var isUpdated = _this3.updateList(data, ['display']);
	                !isUpdated && _this3.updateCacheList(data, ['display']);
	            });

	            /**
	             * 群成员头像变更
	             * @callback data[i] {memberid, tribeid, portrait}
	             */
	            this.binder.on('im.tribemgr.onTribeMemberPortraitChange', function (data) {
	                if (!data || !data.length) return;
	                var tribeid = _this3.state.tribeid;

	                data.forEach(function (item) {
	                    if (item.tribeid.tribeid !== tribeid) return;
	                    item.userid = item.memberid;
	                    item.memberid = item.userid.appkey + item.userid.nick;

	                    var isUpdated = _this3.updateList(item, ['portrait']);
	                    !isUpdated && _this3.updateCacheList(item, ['portrait']);
	                });
	            });

	            /**
	             * 群成员等级变化
	             * @callback data {tribeid, actor, recipient, level}
	             *           level: 1 群主, 2: 管理员, 3: 成员, 5: 游客
	             */
	            this.binder.on('im.tribemgr.onTribeMemberChgLevel', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                data.memberid = data.recipient.appkey + data.recipient.nick;

	                _this3.updateLevel(data);
	            });

	            /**
	             * 群退出
	             * @callback data {tribeid, actor}
	             */
	            this.binder.on('im.tribemgr.onQuitTribe', function (data) {
	                if (!data || !data.actor) return;
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                _this3.handleRemoveMember(data.actor.appkey + data.actor.nick);
	            });

	            /**
	             * 群被踢
	             * @callback data {tribeid, actor, recipient}
	             */
	            this.binder.on('im.tribemgr.onTribeKickMember', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                _this3.handleRemoveMember(data.recipient.appkey + data.recipient.nick);
	            });

	            /**
	             * 群转移
	             * 将被转移者level提升为OWNER，转移者level降为MEMBER
	             * @callback data {tribeid, actor, recipient}
	             */
	            this.binder.on('im.tribemgr.onTribeTransfer', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                // 转移者(原群主)
	                _this3.updateLevel({
	                    memberid: data.actor.appkey + data.actor.nick,
	                    level: LEVELS.MEMBER
	                });
	                // 被转移者
	                _this3.updateLevel({
	                    memberid: data.recipient.appkey + data.recipient.nick,
	                    level: LEVELS.OWNER
	                });
	            });
	            ...
	            /**
	             * 群加入
	             * 先在成员列表中遍历，如果能找到，则直接替换数据，否则push
	             * @callback data {tribeid, actor, info}
	             */
	            this.binder.on('im.tribemgr.onJoinTribe', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;
	                data.info.memberid = data.info.appkey + data.info.nick;
	                var target = data.info;

	                var flag = undefined;
	                [_this3.list, _this3.cacheList].some(function (list) {
	                ...
	                // 如果未找到，则push到列表
	                if (!flag) {
	                    _this3.list.push(data.info);
	                } else if (target.memberid === _this3.loginId) {
	                    _this3.setState({ role: LEVELS.MEMBER });
	                }

	                _this3.setState({ list: _this3.list });
	                clearTimeout(_this3.updateTimmer);
	                _this3.updateTimmer = null;
	            });
	             ...
	             * @callback data {tribeid, actor}
	             */
	            this.binder.on('im.tribemgr.onClearTribeVisitor', function (data) {
	                var tribeid = _this3.state.tribeid;

	                if (data.tribeid.tribeid !== tribeid) return;

	                var count = 0;
	                [_this3.list, _this3.cacheList].forEach(function (list) {
	                    if (!list || !list.length) return;
	                    for (var i = 0, j = list.length; i < j; i++) {
	                        if (list[i].level === LEVELS.VISITOR) {
	                            list.splice(i--, 1);
	                            j--;
	                            count++;
	                        }
	                ...
	                // 如果此时上屏的列表人数少于30个, 则从未上屏的列表中补齐30个
	                if (_this3.list.length < 30 && _this3.cacheList && _this3.cacheList.length) {
	                    var list = _this3.cacheList.splice(0, 30 - _this3.list.length);
	            ...
	            // 主面板打开 主面板关闭时不能移除这个事件，所以不能用this.binder
	            _aliImSdk2['default'].on('im.uiutil.onMainPanelStatusChange', function (data) {
	                if (_this3.pageVisiable === data.bVisable) return;
	                _this3.pageVisiable = data.bVisable;
	                if (data.bVisable) {
	                    location.reload();
	                } else {
	                    _this3.binder.clear();
	        ...
	        value: function handleRemoveMember(memberid) {
	            var _this4 = this;
	            ...
	            // 如果没找到,那么再从未上屏的列表中寻找
	            if (!memberItem && this.cacheList && this.cacheList.length) {
	                this.cacheList.some(function (item, index) {
	                    if (item.memberid === memberid) {
	                        memberItem = item;
	                        _this4.cacheList.splice(index, 1);
	                        return true;
	                    }
	                });
	            }
	            ...
	            // 如果此时上屏的列表人数少于30个, 则从未上屏的列表中补齐30个
	            if (this.list.length < 30 && this.cacheList && this.cacheList.length) {
	                var list = this.cacheList.splice(0, 30 - this.list.length);
	                this.list = this.list.concat(list);
	            }

	            this.setState({ list: this.list });
	            // 清除timeout,因为此时已经更新了this.list
	            clearTimeout(this.updateTimmer);
	            this.updateTimmer = null;
	        }
	        ...
	        key: 'updateLevel',
	        value: function updateLevel(data) {
	            var _this5 = this;

	            var cacheItem = null;
	            var isChange = false;

	            var memberItem = this.list.find(function (item) {
	                return item.memberid === data.memberid;
	            });
	            // 如果在上屏的列表中找不到,在缓存的列表中寻找
	            if (!memberItem && this.cacheList && this.cacheList.length) {
	                this.cacheList.some(function (item, index) {
	                    if (item.memberid === data.memberid) {
	                        cacheItem = _this5.cacheList.splice(index, 1);
	                        return true;
	                    }
	                });
	            }
	            ...
	            if (memberItem) {
	                isChange = true;
	                memberItem.level = data.level;
	            } else if (cacheItem) {
	                // 如果是在缓存的列表中找到的,则将改成员加到上屏列表中,更新
	                isChange = true;
	                cacheItem.level = data.level;
	            ...
	            if (isChange) {
	                ...
	                if (data.memberid === this.loginId) {
	                    this.setState({ role: data.level });
	                }
	            }
	        }
	    }, {
	        key: 'updateList',
	        value: function updateList(data, keys) {
	            var _this6 = this;

	            var list = this.state.list;

	            var memberItem = list.find(function (item) {
	                return item.memberid === data.memberid;
	            });

	            if (!memberItem) return false;
	            keys.forEach(function (k) {
	                return memberItem[k] = data[k];
	            });

	            if (this.updateTimmer) return false;

	            // 更新5s内的变化,否则大群的话,变化会非常频繁,影响性能
	            this.updateTimmer = setTimeout(function () {
	                _this6.updateTimmer = null;
	                _this6.setState({ list: _this6.list });
	            }, 5000);
	        ...
	        key: 'updateCacheList',
	        value: function updateCacheList(data, keys) {
	            if (!this.cacheList || !this.cacheList.length) return false;

	            var memberItem = this.cacheList.find(function (item) {
	                return item.memberid === data.memberid;
	            });
	            if (!memberItem) return false;
	        ...
	        value: function setActive(uid) {
	            ...
	            if (user) {
	                ...
	                var u = undefined;
	                this.cacheList && this.cacheList.forEach(function (item, index) {
	                    if (item.memberid === uid) {
	                        u = _this7.cacheList.splice(index, 1);
	                    }
	                });
	        ...
	        key: 'getBulletinSrc',
	        value: function getBulletinSrc(tribeid) {
	            var _this8 = this;

	            _aliImSdk2['default'].invoke('im.tribemgr.GetTribeBulletin', {
	                tribeid: tribeid
	            }).then(function (url) {
	                _this8.setState({ bulletinSrc: url });
	            });
	        }
	        ...
	        key: 'getMemberList',
	        value: function getMemberList(tribeid, stype) {
	            var _this9 = this;

	            if (!tribeid) return;
	            var isTribe = this.isTribe(stype);

	            var api = isTribe ? 'im.tribemgr.GetTribeMemberList' : 'im.tribemgr.GetMultiChatMembers';
	            _aliImSdk2['default'].invoke(api, { tribeid: tribeid }).then(function (result) {
	                _this9.formatData(result);
	            });
	        }
	    }, {
	        key: 'formatData',
	        value: function formatData(data) {
	            if (!(data && data.memberlist && data.memberlist.length)) return;

	            data.memberlist.forEach(function (item) {
	                item.memberid = item.appkey + item.nick;
	                // level: 1 群主, 2: 管理员, 3: 成员, 5: 游客
	                item.level = item.level === undefined ? LEVELS.MEMBER : item.level;
	            });
	            var memberlist = data.memberlist.slice(0); // 复制一份数据

	            this.total = data.total;
	            this.online = data.online;
	            this.list = memberlist.splice(0, 30);
	            this.cacheList = memberlist;
	        ...
	        key: 'getMoreMember',
	        value: function getMoreMember() {
	            window.DEBUG && console.log('getMoreMemeber-------', this.state.tribeid);
	            if (this.cacheList.length) {
	                var l = this.cacheList.splice(0, 30);
	                this.list = [].concat(_toConsumableArray(this.list), _toConsumableArray(l));
	        ...
	        key: 'isTribe',
	        value: function isTribe(stype) {
	            return (stype || this.state.stype) === 'TM';
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            var _state = this.state;
	            var bulletinSrc = _state.bulletinSrc;
	            var tribeid = _state.tribeid;
	            var list = _state.list;

	            return _react2['default'].createElement(
	                'div',
	                { className: 'right-side-bar' },
	                this.isTribe() ? _react2['default'].createElement(_bulletin2['default'], { src: bulletinSrc }) : null,
	                _react2['default'].createElement(_listTitle2['default'], this.state),
	                this.isTribe() ? _react2['default'].createElement(_search2['default'], { tribeid: tribeid, setActive: this.setActive }) : null,
	                this.isTribe() ? _react2['default'].createElement(_memberList2['default'], { list: list, tribeid: tribeid, loginId: this.loginId, getMoreList: this.getMoreMember }) : _react2['default'].createElement(_multiMemberList2['default'], { list: list, tribeid: tribeid, loginId: this.loginId })
	            );
	        }
	    }]);
	...
	var Title = (function (_React$Component) {
	    ...
	    _createClass(Title, [{
	        ...
	        value: function clickHandler() {
	            // 邀请加入群
	            _aliImSdk2['default'].invoke('im.uiutil.InviteTribeMember', {
	        ...
	        value: function render() {
	            ...
	            if (stype === 'TM') {
	                title = 'Член группы(' + online + '/' + total + ')';
	            } else if (stype === 'Multi') {
	                title = 'Дискуссионная группа(' + total + ')';
	            }

	            var authAdd = stype === 'TM' && (role === LEVELS.OWNER || role === LEVELS.ADMIN || role === LEVELS.MEMBER);
	            ...
	            return _react2['default'].createElement(
	                ...
	                authAdd ? _react2['default'].createElement(
	                    'span',
	                    { className: 'add-member', onClick: this.clickHandler.bind(this), title: 'Пригласите друзей присоединиться к группе' },
	                    _react2['default'].createElement('i', { className: 'iconfont icon-add-friends' })
	                ) : null
	            );
	        }
	    }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _member = __webpack_require__(226);
	...
	var MemberList = (function (_React$Component) {
	    ...
	    _createClass(MemberList, [{
	        ...
	        value: function bindEvents() {
	            ...
	            this.con = _reactDom2['default'].findDOMNode(this);

	            this.con.addEventListener('scroll', function () {

	                if (!_this.conHeight) {
	                    _this.conHeight = _this.con.clientHeight;
	                }

	                if (!_this.fetching && _this.con.scrollHeight - _this.con.scrollTop - _this.conHeight < 20) {
	                    _this.fetching = true;
	                    _this.props.getMoreList();
	                }
	            });
	        ...
	        value: function render() {
	            ...
	            var memberNode = undefined,
	                isOffline = undefined;
	            var _props = this.props;
	            var list = _props.list;
	            var loginId = _props.loginId;
	            var tribeid = _props.tribeid;

	            list.forEach(function (m) {
	                var memberid = m.memberid;
	                isOffline = m.status === 0;
	                memberNode = _react2['default'].createElement(_member2['default'], _extends({ key: memberid }, m, { tribeid: tribeid, loginId: loginId }));

	                switch (m.level) {
	                    case LEVELS.OWNER:
	                        roleOwners.push(memberNode);
	                        break;
	                    case LEVELS.ADMIN:
	                        roleMasters.push(memberNode);
	                        break;
	                    case LEVELS.MEMBER:
	                        memberid === loginId ? roleSelf.push(memberNode) : roleMember.push(memberNode);
	                        break;
	                    case LEVELS.VISITOR:
	                        roleVisitor.push(memberNode);
	                        break;
	                    case LEVELS.SEARCH:
	                        // 搜索结果
	                        searchList.push(memberNode);
	                        break;
	                    default:
	                        roleUnknown.push(memberNode);
	                        break;
	                }
	            });

	            listNodes = [].concat(roleOwners, roleMasters, roleSelf, searchList, roleMember, roleVisitor, roleUnknown);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _$ROOTCOnlinestatus = __webpack_require__(171);

	var _$ROOTCOnlinestatus2 = _interopRequireDefault(_$ROOTCOnlinestatus);
	    ...
	    _createClass(Member, [{
	        ...
	        value: function handleContextMenu(e) {
	            ...
	            var params = {
	                ...
	                hititem: {
	                    type: 3,
	                    jvalue: JSON.stringify({
	                        tribeid: { tribeid: tribeid },
	                        memberid: { appkey: appkey, nick: nick }
	                    })
	                }
	            };

	            _aliImSdk2['default'].invoke('im.uiutil.ShowTribeMemberListMenu', params);
	        }
	    }, {
	        key: 'handleDblClick',
	        value: function handleDblClick() {
	            var _props2 = this.props;
	            var appkey = _props2.appkey;
	            var nick = _props2.nick;
	            var memberid = _props2.memberid;
	            var loginId = _props2.loginId;

	            if (memberid === loginId) return;

	            _aliImSdk2['default'].invoke('im.tribemgr.SelectTribeSearchResult', { appkey: appkey, nick: nick });
	        }
	    }, {
	        key: 'handleMouseEnter',
	        value: function handleMouseEnter(ev) {
	            clearTimeout(this.mouseEnterTimmer);
	            var _props3 = this.props;
	            var appkey = _props3.appkey;
	            var nick = _props3.nick;

	            var x = ev.pageX,
	                y = ev.pageY;
	            this.mouseEnterTimmer = setTimeout(function () {
	                _aliImSdk2['default'].invoke('im.uiutil.ShowTribeMemberNameCard', {
	                    cid: { appkey: appkey, nick: nick },
	                    x: x, y: y
	                });
	            }, 500);
	        }
	        ...
	        value: function componentDidMount() {
	            if (this.props.isActive) {
	                var e = document.getElementById(this.props.memberid);
	                e.focus();
	                e.scrollIntoView();
	            }
	        }
	        ...
	        value: function render() {
	            ...
	            var nick = _props4.nick;
	            var memberid = _props4.memberid;
	            var display = _props4.display;
	            var status = _props4.status;
	            var portrait = _props4.portrait;
	            var level = _props4.level;

	            var clazz = (0, _classnames2['default'])('member', { offline: status === 0 });
	            var levelClazz = (0, _classnames2['default'])('member-role', ROLE_MAP[level]);
	            var icon = status && status != 1 && _$ROOTCOnlinestatus2['default'][status].text === 'Войти с телефона' && _$ROOTCOnlinestatus2['default'][status].icon;
	            var title = ROLE_TITLE[level];

	            return _react2['default'].createElement(
	                'li',
	                { className: clazz, tabIndex: '-1',
	                    onContextMenu: this.handleContextMenu,
	                    onDoubleClick: this.handleDblClick, id: memberid, title: nick },
	                _react2['default'].createElement(
	                    'span',
	                    { 'data-uid': memberid, className: levelClazz, title: title },
	                    title ? _react2['default'].createElement('i', { className: 'iconfont icon-people' }) : null
	                ),
	                _react2['default'].createElement(
	                    'div',
	                    { className: 'member-avatar-wrap', onMouseEnter: this.handleMouseEnter, onMouseLeave: this.handleMouseLeave },
	                    _react2['default'].createElement('img', { src: portrait, className: 'member-avatar', 'data-uid': memberid }),
	                    icon ? _react2['default'].createElement('img', { className: 'member-status-icon', src: icon }) : ''
	                ),
	                _react2['default'].createElement(
	                    'span',
	                    { className: 'member-nick', 'data-uid': memberid, className: 'member-nick' },
	                    display || nick
	                )
	            );
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _multiMember = __webpack_require__(228);

	var _multiMember2 = _interopRequireDefault(_multiMember);
	        ...
	        value: function render() {
	            ...
	            return _react2['default'].createElement(
	                ...
	                _react2['default'].createElement(
	                    'ul',
	                    { className: 'inner-list' },
	                    this.props.list.map(function (m) {
	                        return _react2['default'].createElement(_multiMember2['default'], _extends({ key: m.memberid }, m, { tribeid: _this.props.tribeid, loginId: _this.props.loginId }));
	                    })
	                )
	            );
	        }
	...
	var Member = (function (_React$Component) {
	    ...
	    _createClass(Member, [{
	        ...
	        value: function handleContextMenu(e) {
	            ...
	            _aliImSdk2['default'].invoke('im.uiutil.ShowMultiMemberListMenu', params);
	        }
	    }, {
	        key: 'handleDblClick',
	        value: function handleDblClick() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _searchTrigger = __webpack_require__(230);

	var _searchTrigger2 = _interopRequireDefault(_searchTrigger);

	var _searchInput = __webpack_require__(231);

	var _searchInput2 = _interopRequireDefault(_searchInput);

	var _searchSuggest = __webpack_require__(232);
	...
	var Search = (function (_React$Component) {
	    ...
	    _createClass(Search, [{
	        ...
	        value: function doSearch(val) {
	            ...
	            // 如果为空，清空数据
	            if (!val) return this.doClear();

	            _aliImSdk2['default'].invoke('im.tribemgr.SearchTribeMember', {
	                key: val,
	                tribeid: { tribeid: this.props.tribeid }
	            }).then(function (data) {
	                data = data || [];
	                data.forEach(function (item) {
	                    return item.uid = item.appkey + item.nick;
	                });
	                ...
	                if (data.length) {
	                    // 默认选中第一个
	                    data[0].active = true;
	                    _this.list = data.splice(0, 20);
	                    _this.cacheList = data;
	                }

	                _this.setState({ resultList: _this.list });
	            });
	        }
	        ...
	        value: function selectSearchItem(keyCode) {
	            var _this2 = this;
	            ...
	            // enter键,直接定位到该联系人在群成员列表的位置
	            if (keyCode === 13) {
	                var f = list.find(function (item) {
	                    return item.active;
	                });
	                this.setActive(f);
	            }
	            // 上下键
	                    ...
	                    list.forEach(function (item, index) {
	                        ...
	                        if (item.active) {
	                            ...
	                            if (keyCode === 38) {
	                                ...
	                                index = index + 1 < _this2.list.length + _this2.cacheList.length ? index + 1 : 0;
	                                if (index + 5 > _this2.list.length && _this2.cacheList.length) {
	                                    var l = _this2.cacheList.splice(0, 20);
	                                    _this2.list = [].concat(_toConsumableArray(_this2.list), _toConsumableArray(l));
	                                }
	                            }
	                    ...
	                    isChange && this.setState({
	                        resultList: list
	                    });

	                    isChange && document.getElementById('J_si_' + isOk).scrollIntoViewIfNeeded();
	                }
	        }
	    }, {
	        key: 'getMoreList',
	        value: function getMoreList() {
	            ...
	            var list = this.cacheList.splice(0, 20);
	            this.list = this.list.concat(list);
	        ...
	        value: function render() {
	            return _react2['default'].createElement(
	                'div',
	                { className: 'search' },
	                _react2['default'].createElement(_searchTrigger2['default'], { setTrigger: this.setTrigger, isShow: this.state.isShow }),
	                _react2['default'].createElement(_searchInput2['default'], {
	                    isShow: this.state.isShow,
	                    doSearch: this.doSearch,
	                    selectSearchItem: this.selectSearchItem,
	                    onClear: this.doClear }),
	                _react2['default'].createElement(_searchSuggest2['default'], { list: this.state.resultList,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SearchTrigger = (function (_React$Component) {
	    ...
	    function SearchTrigger() {
	        ...
	        _get(Object.getPrototypeOf(SearchTrigger.prototype), 'constructor', this).apply(this, arguments);
	    }
	    ...
	    _createClass(SearchTrigger, [{
	        ...
	        value: function render() {
	            return _react2['default'].createElement(
	                'span',
	                { className: 'search-trigger', onClick: this.clickHandler.bind(this), title: this.props.isShow ? 'Свернуть окно поиска участников группы' : 'Развернуть окно поиска участников группы' },
	                _react2['default'].createElement('i', { className: 'iconfont icon-sousuo' })
	            );
	        }
	    }]);
	...
	var Search = (function (_React$Component) {
	    ...
	    _createClass(Search, [{
	        ...
	        value: function doSearch(ev) {
	            ...
	            // 除了上下左右enter键外,均进搜索
	            if (keyCode !== 13 && keyCode !== 38 && keyCode !== 39 && keyCode !== 40 && keyCode !== 37) {
	                var value = ev.target.value;
	                this.props.doSearch(value.trim());
	            } else {
	                this.props.selectSearchItem(keyCode);
	            }
	        }
	    }, {
	        key: 'handleClear',
	        ...
	        value: function render() {
	            return _react2['default'].createElement(
	                'div',
	                { className: 'search-input ' + (this.props.isShow ? 'show' : '') },
	                _react2['default'].createElement(
	                    'span',
	                    { className: 'search-label' },
	                    _react2['default'].createElement('i', { className: 'iconfont icon-sousuo' })
	                ),
	                _react2['default'].createElement('input', { className: 'search-inputer',
	                    type: 'text',
	                    ref: 'searchInput',
	                    placeholder: 'Найдите друзей в группе',
	                    onKeyUp: this.doSearch }),
	                _react2['default'].createElement(
	                    'span',
	                    { className: 'search-input-clear',
	                        onClick: this.handleClear },
	                    _react2['default'].createElement('i', { className: 'iconfont icon-guanbi' })
	                )
	            );
	        }
	    }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _searchItem = __webpack_require__(233);
	    ...
	    _createClass(Search, [{
	        ...
	        value: function bindEvents() {
	            ...
	            this.con.addEventListener('scroll', function () {
	                ...
	                if (!_this.fetching && _this.con.scrollHeight - _this.conHeight - _this.con.scrollTop < 10) {
	                    _this.props.getMoreList();
	                    _this.fetching = true;
	                }
	            });
	        ...
	        value: function render() {
	            ...
	            var cls = 'search-suggest ' + (this.props.list.length ? 'show' : ''),
	                list = [];

	            this.props.list.forEach(function (item) {
	                list.push(_react2['default'].createElement(_searchItem2['default'], _extends({ key: item.uid }, item, { setActive: _this2.props.setActive.bind(_this2, item) })));
	            });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SearchItem = (function (_React$Component) {
	    ...
	    function SearchItem(props) {
	        ...
	        _get(Object.getPrototypeOf(SearchItem.prototype), 'constructor', this).call(this, props);
	    ...
	    _createClass(SearchItem, [{
	        ...
	        key: 'render',
	        value: function render() {
	            var cls = (0, _classnames2['default'])('search-item', { 'item-active': this.props.active, 'offline': this.props.state == 0 });
	            return _react2['default'].createElement(
	                'div',
	                { id: 'J_si_' + this.props.uid, className: cls, onClick: this.setActive },
	                _react2['default'].createElement('img', { src: this.props.portrait }),
	                _react2['default'].createElement(
	                    'span',
	                    null,
	                    this.props.nick
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Bulletin = (function (_React$Component) {
	    ...
	    function Bulletin() {
	        ...
	        _get(Object.getPrototypeOf(Bulletin.prototype), 'constructor', this).apply(this, arguments);
	    }
	    ...
	    _createClass(Bulletin, [{
	        ...
	        value: function render() {
	            ...
	            return _react2['default'].createElement('iframe', { src: src, frameBorder: '0', width: '100%', height: '180', id: 'J_bulletinFrame' });
	        }
	    }]);
