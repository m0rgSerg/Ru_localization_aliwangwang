...
/******/ 			script.src = __webpack_require__.p + "" + {"0":"db9c2629acee47818ee2"}[chunkId] + ".js";
...
/***/ function(module, exports, __webpack_require__) {
	...
	!(function(global, factory) {
	    ...
	    // 调试模式
	    var IS_DEBUG = (global.location && location.href && /debug=true/i.test(location.href)) || (global.localStorage && !!localStorage.getItem('DEBUG'));
	    ...
	    var isMac, defaultNameSpace, defaultInvoke, nativeRequestMethod, defaultRequestMethod,
	        createSequenceId, eventRegister, eventNameSpace, eventUnregister, boradCase;

	    function initEnv() {
	        isMac = global.webkit && global.webkit.messageHandlers && global.webkit.messageHandlers.WorkBench && global.webkit.messageHandlers.WorkBench;
	        defaultNameSpace = !isMac ? global.workbench : global.webkit;
	        defaultInvoke = !isMac ? 'invoke' : 'postMessage';
	        nativeRequestMethod = !isMac ? defaultNameSpace.httprequest : defaultNameSpace.messageHandlers.WorkBench;
	        defaultRequestMethod = !isMac ? 'CommitHttpRequest' : 'httpRequestInvoke';
	        createSequenceId = !isMac ? global.workbench.createSequenceId : __GenerateUUID;
	        eventNameSpace = !isMac ? defaultNameSpace.im : defaultNameSpace.messageHandlers.WorkBench;
	        eventRegister = !isMac ? defaultNameSpace.im.event_register : defaultNameSpace.messageHandlers.WorkBench.postMessage;
	        eventUnregister = !isMac ? defaultNameSpace.im.event_unregister : defaultNameSpace.messageHandlers.WorkBench.postMessage;
	        boradCase = !isMac ? defaultNameSpace.im.event_notify : defaultNameSpace.messageHandlers.WorkBench.postMessage;
	    }

	    function createArgs(id, cmd, param, other) {
	        return !isMac ?
	            (other ? [id, cmd, param] : [id, cmd, param, other]) :
	            (other ? [{ sid: id, method: cmd, params: param }] : [{ sid: id, method: cmd, params: param, other: other }]);
	    }
	    /*--------------------------  TASK  ----------------------------*/
	    ...
	    function Task(namespace, config) {
	        ...
	        !createSequenceId && initEnv();

	        this.id = __GenerateUUID();
	    ...
	    Task.prototype = {
	        ...
	        run: function() {
	            ...
	            var args = createArgs(this.id, this.cmd, param, this.other);

	            try {
	                this.namespace[defaultInvoke].apply(this.namespace, args);
	            } catch (err) {
	        ...
	        request: function (requestMethod) {
	            requestMethod = requestMethod || defaultRequestMethod;

	            var args = createArgs(this.id, requestMethod, JSON.stringify({
	                url: this.config.url,
	            ...
	            try {
	                nativeRequestMethod[defaultInvoke].apply(nativeRequestMethod, args);
	            } catch (err) {
	    ...
	    SDK.invoke = function(fn, config) {
	        ...
	        try {
	            fn.forEach(function(n) {
	                namespace[n] ? (namespace = namespace[n]) : (cmd = n + '.' + cmd);
	            });
	        } catch (err) {
	    ...
	    SDK.register = function (name) {
	        ...
	        // 客户端方法支持传入数组，直接注册多个事件
	        var args = !isMac ? createArgs('', JSON.stringify(name), '') : createArgs('', 'registerEvent', JSON.stringify(name));
	        eventRegister.apply(eventNameSpace, args);

	        try {
	            name.forEach(function (n) {
	                REGISTER_CACHE[n] = true;
	            });
	        } catch (err) {
	            _errorLog('%cError in SDK.register', 'color:red;', name);
	        }
	    ...
	    SDK.unregister = function (name) {
	        ...
	        // 客户端方法支持传入数组，直接解绑多个事件
	        var args = !isMac ? createArgs('', JSON.stringify(name), '') : createArgs('', 'unregisterEvent', JSON.stringify(name));
	        eventUnregister.apply(eventNameSpace, args);
	        ...
	        try {
	            ...
	            _errorLog('%cError in SDK.' + '', 'color:red', err);
	        }
	     ...
	     * @param   {Function}    [callback]        [Необязательно] Обратный вызов триггеров событий
	     * @param   {Boolean}     [canUnregister=true]      [Необязательно] Можно ли отменить регистрацию событий самостоятельно, по умолчанию true
	     * @return  {Object}      SDK
	     * @example
	     * SDK.off();
	    ...
	    SDK.broadcast = function(event, data) {
	        ...
	        var args = !isMac ? createArgs('', event, param) : createArgs('', 'eventNotify', JSON.stringify({
	            event: event,
	            param: param
	        }));

	        boradCase.apply(null, args);
	    };
	    ...
	    SDK._overrideWithPromise = function () {
	        ...
	        SDK.ajaxSetup = {
	            // ajax После возврата метода бизнес-уровень Правила проверки, используются только для управления данными
